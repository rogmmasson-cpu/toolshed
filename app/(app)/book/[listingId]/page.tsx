...
import { Shield, ChevronRight, Check, Zap, CreditCard } from 'lucide-react'
...
export default function BookingPage() {
  ...
  const rawListing = MOCK_LISTINGS.find((l) => l.id === listingId)
  const rawOwner = rawListing ? MOCK_USERS.find((u) => u.id === rawListing.ownerId) : null
  if (!rawListing || !rawOwner) return <div className="container-app py-20 text-center text-gray-500">Listing not found.</div>

  const listing: ListingWithOwner = {
    ...rawListing,
    owner: { id: rawOwner.id, name: rawOwner.name, avatarUrl: rawOwner.avatarUrl, trustScore: rawOwner.trustScore, badges: rawOwner.badges, responseRate: rawOwner.responseRate },
  }
  ...
  async function handleConfirm() {
    ...
    router.push(`/book/${listingId}/confirmation?start=${startDate}&end=${endDate}`)
  }
        ...
        <div className="lg:col-span-2">
          ...
          {step === 2 && (
            <div className="card p-6 space-y-5">
              ...
              <div className={cn('border-2 rounded-2xl p-5 cursor-pointer transition-all', insuranceEnrolled ? 'border-forest-400 bg-forest-50' : 'border-gray-200 hover:border-gray-300')}
                ...
                <ul className="mt-3 space-y-1 text-xs text-gray-500 ml-13">
              ...
              <div className={cn('border-2 rounded-2xl p-5 cursor-pointer transition-all', !insuranceEnrolled ? 'border-gray-400 bg-gray-50' : 'border-gray-200 hover:border-gray-300')}
